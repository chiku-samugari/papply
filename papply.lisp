...
(defmacro apapply-function (fn &rest args)
  (with-gensyms (more-args)
    `(lambda (,@(anaphora-list args) &rest ,more-args)
       (declare (ignorable ,more-args))
...
(defmacro apapply-form ((op &rest forms))
  (if (and (symbolp op)
    ...
    `(lambda (,@(anaphora-list forms))
       (,op ,@forms))
    (with-gensyms (more-args)
      `(lambda (,@(anaphora-list forms) &rest ,more-args)
         (declare (ignorable ,more-args))
         (apply (function ,op) ,@forms ,more-args)))))

(defmacro apapply (fn &rest args)
  (if (or (atom fn)
          (and (consp fn) (eq (car fn) 'cl:function)))
    `(apapply-function ,fn ,@args)
    `(apapply-form ,fn)))
